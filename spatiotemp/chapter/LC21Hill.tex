...
is an $[\cl{}\!\times\!\cl{}]$ matrix, which acts on an \cl{}\dmn\ linear space. 
For an infinitely long {\lattstate}, the {\jacobianOrb} is an infinite\dmn\ linear operator. 
...
Since the {\jacobianOrb} only acts on the field defined on the integer lattice, the periodic
function $u_k(\zeit)$ is a constant on the lattice. And the eigenstate $\psi_k$ is:
\beq
(\psi_{k})_\zeit = \psi_{k}(\zeit) = e^{\mathrm{i}k\zeit}
    \,,\quad \zeit \in \integers
 \,.
\ee{BlochLatt}
So the eigenstates are plane waves on the lattice, in agreement with the fact that eigenstates
of translation operators are plane waves.
Given the periodicity of the integer lattice, we can find all eigenvalues with the wave vector
$k$ confined in the first Brillouin zone: from $k=-\pi$ to $k=\pi$. Let the {\jacobianOrb}
\refeq{tempCatFix} act on the eigenstates \refeq{BlochLatt}, we find that the 
eigenvalues of the {\jacobianOrb} are:
\beq
E_k = \jMorb \psi_k = s - 2\cos\left(\frac{2\pi k}{\cl{}}\right) \,.
\ee{EigenvalueSpectrum}
The eigenvalue with wave vector $k$ in the first Brillouin zone is plotted in 
\reffig{fig:LC21emplattBand} (a).
