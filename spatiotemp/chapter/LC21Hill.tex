...
% siminos/figSrc/han/Mathematica/LC21Figure/Cyc2FundParallel.nb
\begin{figure}
  \centering
(a)~\includegraphics[width=0.25\textwidth]{BernCyc2Unit}%{BernCyc2JacobUnit}
~~~~~~
(b)~\includegraphics[width=0.30\textwidth]{BernCyc2FundParallel}%{BernCyc2JacobPart}
...
% siminos/figSrc/han/Mathematica/LC21Figure/Cyc3FundParallel.nb
\begin{figure}
  \centering
(a)~\includegraphics[width=0.38\textwidth]{CatCyc2FundParallel}%{catCyc2JacobUnit}
~~~
(b)~\includegraphics[width=0.34\textwidth]{CatCyc3FundParalllel}%{PCLength3Counting}
...
As the period of the {\lattstate} getting longer, the {\jacobianOrb}
becomes a hign\dmn\ matrix and the {\HillDet} becomes hard to compute.
However, if the {\jacobianOrb} is invariant under time translation, the 
{\HillDet} can be easily computed using crystallographer's favorite tool, 
the discrete Fourier transform. The Fourier discretization
approach goes all the way back to Hill's 1886 paper\rf{Hill86}.

{\JacobianOrbs} of the {temporal Bernoulli} \refeq{bernFixPoint} and 
{\templatt} \refeq{tempCatFix} consist of only identity matrix and cyclic
shift matrix, whose eigenvectors are discrete Fourier basis 
\refeq{FourierModes}. So {\jacobianOrbs} of the {temporal Bernoulli} and 
{\templatt} can be diagonalized by the discrete Fourier transform. 
In the space of reciprocal {\lattstate}s, {\jacobianOrbs} \refeq{bernFixPoint}
and \refeq{tempCatFix} are diagonal matrices, which will act locally on
each reciprocal lattice site. The diagonal elements are eigenvalues of
{\jacobianOrbs}, which for the {temporal Bernoulli} are:
\bea
...
\label{BernEigen}
\eea
and for the \templatt\ are:
\bea
(-\shift+{s}\,\id-\shift^{-1})\,\tilde{e}_k
=
\left[{s} - 2\cos\left(\frac{2\pi k}{\cl{}}\right)\right]\,\tilde{e}_k \, .
\label{CatEigen}
\eea
\HillDet s are products of eigenvalues. For the {temporal Bernoulli},
the {\HillDet} of a period-\cl{} {\lattstate} is:
\bea
\Det({s}\id - {\shift})
=
...
\,.
\eea
For the {\templatt}, the {\HillDet} of a period-\cl{} {\lattstate} is:
\bea
\Det(-\shift+{s}\,\id-\shift^{-1})
    =
\prod_{k=0}^{\cl{}-1} \left[{s} - 2\cos\left(\frac{2\pi k}{\cl{}}\right)\right]
    =
2 T_{\cl{}} \left({s}/{2}\right) - 2 \, .
\eea

\subsection{Eigenvalue spectrum of the {\jacobianOrb}}
...
In the previous example, the {\jacobianOrb} \refeq{jacobianOrb} of a period-\cl{} {\lattstate}
is an $[\cl{}\!\times\!\cl{}]$ matrix.
For an infinite {\lattstate}, the {\jacobianOrb} is an infinite\dmn\ linear operator. 

Consider the {\templatt} as an example. The infinite\dmn\ {\jacobianOrb} still has the form
\refeq{tempCatFix}, where the {\shiftOp} $\shift$ implements the shift operation on
the infinite lattice. The {\jacobianOrb} commutes with the {\shiftOp}, which means that the
{\jacobianOrb} has the translation symmetry, and the translation symmetry group is given by the
integer lattice. By the Bloch theorem (or Floquet theorem), we know that the eigenstates of 
the linear operator $\jMorb$ have the form of a plane wave times a periodic function:
\beq
\psi_{k}(\zeit) = e^{\mathrm{i}k\zeit} u_{k}(\zeit) \,,
\ee{BlochTheorem}
where $u_k(\zeit)$ is a periodic function with period 1.
The {\jacobianOrb} only acts on the field defined on the integer lattice, on which the periodic
function $u_k(\zeit)$ is a constant. So the eigenstates $\psi_k$ are:
\beq
(\psi_{k})_\zeit = \psi_{k}(\zeit) = e^{\mathrm{i}k\zeit}
    \,,\quad \zeit \in \integers
 \,,
\ee{BlochLatt}
which are plane waves on the lattice, in agreement with the fact that eigenstates
of translation operators are plane waves.
Given the periodicity of the integer lattice, 
the wave vector $k$ of the eigenstate $\psi_k$ can always be confined to
the first Brillouin zone: from $k=-\pi$ to $k=\pi$. Let the {\jacobianOrb}
\refeq{tempCatFix} act on the eigenstates \refeq{BlochLatt}, we find the 
eigenvalues of the {\jacobianOrb}:
\beq
E_k = \jMorb \psi_k = s - 2\cos k \,.
\ee{EigenvalueSpectrum}
The eigenvalue with wave vector $k$ in the first Brillouin zone is plotted in 
\reffig{fig:LC21emplattBand}\,(a). Note that because the {\jacobianOrb} \refeq{tempCatFix}
has the reflection symmetry, the eigenvalue $E_k$ also has reflection symmetry in the
first Brillouin zone, $E_k=E_{-k}$.

\refeq{EigenvalueSpectrum} contains all possible eigenvalues of the {\jacobianOrb}.
To find eigenvalues of the {\jacobianOrb} of a period-\cl{} {\lattstate}, we need to set the 
periodic boundary condition to the eigenstate:
\beq
\psi_k(\zeit + \cl{}) = \psi_k(\zeit) \,.
\ee{PeriodicEigenstate}
To satisfy this periodic boundary condition, wave vector
$k$ must exist on the reciprocal lattice of the Bravais lattice spanned by \cl{}, \ie,
 $k = 2 \pi m/\cl{}$, where
$m$ is an integer. The eigenvalues of a period-\cl{} {\lattstate} are:
\beq
E_k = \jMorb \psi_k = s - 2\cos k \,,
\qquad k = \frac{2 \pi m}{\cl{}}\,, \quad m \in \integers \,,
\ee{FiniteStateEigenvalue}
in agreement with \refeq{CatEigen}.
The number of the allowed wave vectors in the first Brillouin zone is \cl{}.
Examples of the period-3 and period-4 {\jacobianOrb} are sketched in 
\reffig{fig:LC21emplattBand}\,(b) and (c).
...
\begin{quote}
If you evaluate stability of the 3-term recurrence \refeq{LC21:1dTempFT}
%was \refeq{JiKoKr20(2)} 
on a periodic lattice you get the {\jacobianOrb} $\jMorb$;
