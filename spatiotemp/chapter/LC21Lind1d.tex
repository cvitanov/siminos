...
\refeq{1stepDiffSolu} using the determinant {\jacobianOrb} \jMorb.
The fundamental fact also holds for the time reversal invariant lattice states. 
As an example, consider {\templatt} with $s=3$. Using the
determinant of the {\jacobianOrb} \refeq{1stepDiffSolu} and symmetry reduced {\jacobianOrb}s
\refeq{HillDetSymmReduced}, the number of {\lattstate}s are:
\bea
N_\cl{} &=& \left(\ExpaEig^{\cl{}/2}-\ExpaEig^{-\cl{}/2}\right)^2 \,, \continue
N_{\cl{},0} &=& \ExpaEig^{\cl{}/2}-\ExpaEig^{-\cl{}/2} \, , \qquad \qquad
\cl{} \mbox{ is odd,} \continue
N_{\cl{},0} &=& \sqrt{5}\left(\ExpaEig^{\cl{}/2}-\ExpaEig^{-\cl{}/2}\right) \, , \qquad
\cl{} \mbox{ is even,} \continue
...
generating functions of the {\lattstate} count are:
\bea
...
+ \dots
\eea
which are in agreement with \reftab{tab:lattstateCountCat}.
...
Numbers of period-$\cl{}$ {\lattstate}s of {\templatt} with $s=3$.
