...
Lattice state and {prime} orbit counts for the ${s}=3$ {\templatt}.
$M_\cl{}$ is the number of period-$\cl{}$ prime orbits
(see Bird and Vivaldi\rf{BirViv}).
...
but I do remember all $\ExpaEig^{1/2}$ eventually going away... Never mind.
    ...
    \HL{2021-12-30}{For general $s$ we have:
    $N_{\cl{},0} = \sqrt{(s-2)N_{\cl{}}}$ \refeq{HillDetSymmReduced}. This is in agreement
    with \reftab{tab:lattstateCountCat}.
    }
Numbers of {\lattstate}s with period up to 10 are listed in the
\reftab{tab:lattstateCountCat}. Using \refeq{KLPzetaExp} we have:
\bea
h(t) &=& \sum_{m=1}^{\infty} \left[
       N_{2m-1, 0}\,t^{2m-1}
       + \left(N_{2m,0}+N_{2m,1}\right)\,\frac{ t^{2m}}{2}
