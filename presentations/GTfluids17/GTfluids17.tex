% siminos/presentations/Zagreb17/Zagreb17.tex        pdflatex Zagreb17
% $Author: predrag $ $Date: 2017-09-12 22:53:12 -0400 (Tue, 12 Sep 2017) $

%  started with presentations/Dresden17/Dresden17.tex       2017-04-17
%  started with siminos/presentations/KITP17/UCSB17.tex     2017-02-05
...
\begin{frame}{example : pipe flow
    \footfullcite{science04}%$^,$%
    %\footfullcite{WK09}
}
amazing experimental data! amazing numerics!
\begin{center}
...
\begin{frame}%{plane Couette state space $10^5 \to 3D$}
    {{\scriptsize (click on this link)} \HREF{https://youtu.be/sg6ESlimzQU}{Ritorna Vincitor !}}
\begin{center}
...
\begin{frame}{however : this program has hit a wall}
\color{red}{we cannot find them}
\begin{block}{unable to compute invariant solutions for large spatial
                domains}
{\Large
solutions on large domains are too unstable\footfullcite{WFSBC15}
                  }
\end{block}


\vfill
\color{red}{we cannot name them}
\begin{block}{unable to develop temporal symbolic dynamics for any PDE}
{\Large
solutions on large domains require exponentially many letters\footnote{
Siminos, Gibson, Ding, Budanur, etc etc have thrown in their collective hat
}
                  }
\end{block}
\end{frame}
...
\begin{frame}{large space-time domains}
...
\hfill {\scriptsize \color{orange}
\HREF{http:\\codeinthehole.com/static/tutorial/coherent.html}
...
strategy : first study % dicretized spacetime
...
\begin{frame}{conclusion : the fate of a single kitten} % and free will}
...
\hfill\includegraphics[width=0.45\textwidth]{DawnBishopCats}
%\end{center}
...
\begin{frame}{herding cats : a Euclidean field theory
    %\footfullcite{GHJSC16}%$^,$%
    %\footfullcite{WK09}
}
convert the spatial-temporal differences to % discrete derivatives \bigskip
...
\begin{frame}{compact time, infinite space \KS
    \footfullcite{GuBuCv17}%$^,$\footfullcite{WK09}
}
\bea
    u_\zeit &=&  - u u_\conf
...
\begin{frame}{variational principle for a general flow}
\begin{center}
...
\textcolor{green}{solution} %periodic orbit}
...
an exact spatio-temporal  solution of a field theory
    % \footfullcite{LCC06}%$^,$\footfullcite{Ihara66}
\end{frame}
...
\begin{frame}{towards scalable parallel-in-time turbulent flow simulations
}
\begin{block}{future :}%
...
\\
\HREF{http://ChaosBook.org/course1}{take the course!}
}
\begin{center}
...
\begin{frame}{$1d$ example : variational principle for any {\po}
    \footfullcite{lanVar1}$^,$\footfullcite{CvitLanCrete02}
}
...
\begin{frame}{zeta function for a field theory ? much like Ising model
    \footfullcite{KacWar52}$^,$\footfullcite{Ihara66}
}
