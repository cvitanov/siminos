...
if case==2:
    ...
    def timestep_jacobian(point):
        c=a
        if(Gallas):
            c=1
        A = np.array([[-2*c*point, -1], [1., 0.]])
        eigen=np.linalg.eig(A)
        eigen=np.linalg.eig(A)
        return(A,eigen)
    
    def sqrt_timestep_jacobian(n,point):
        J0,eig=timestep_jacobian(point)
        eigenindex=np.argsort(eig[0])
        eigs=eig[0][eigenindex]
        eigvecs=eig[1][eigenindex]
        sign1=np.sign((1j)**n).real
        sign2=np.sign((-1j)**n).real
        D=np.array([[sign1*np.sqrt(eigs[0]),0],[0,sign2*np.sqrt(eigs[1])]])
        S=np.array([eigvecs[0],eigvecs[1]])
        S1=np.linalg.inv(S)        
        sqrt_J0=np.linalg.multi_dot([S,D,S1])
        return(sqrt_J0)
    ...
    def middle_lattice_state(orbit):
        ...
        symmetric=rolled[2:5]
        return(rolled,symmetric)
    ...
    sym=middle_lattice_state(cycles_of_interest[0])[1]
    Jplus=np.array([[2*sym[0],2,0],[1,2*sym[1],1],[0,1,2*sym[2]+1]])
    sJ0=sqrt_timestep_jacobian(1,sym[0])
    J1=timestep_jacobian(sym[1])[0]
    J2=timestep_jacobian(sym[2])[0]
    Jtime=np.linalg.multi_dot([J2,J1,sJ0])
    a=np.abs(np.linalg.det(Jplus))-np.abs(np.linalg.det(np.identity(2)-Jtime))
    print(a)
