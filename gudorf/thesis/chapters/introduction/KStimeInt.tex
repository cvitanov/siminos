...
$u(x,t)=u(x+\speriod{},t)$, and integrate
\beq
    u_t + u_{x x} + u_{x x x x} + u_x u = 0
    \,,\quad
    x \in [0,\speriod{})
    \label{e-ksL}
\eeq
...
(a) The 1D \KSe\ is usually integrated on a \spt\ cylinder of an
    ...
    $t\in\{-\infty,\infty)$; for an example, see \reffig{f:ks_largeL}.
(b) It is also possible to integrate the equation on a \spt\ cylinder of an
    ...
    ranging over $x\in\{-\infty,\infty)$, see \refsect{sect:KSspaceInt}.
(c) Here we shall seek {\spt}ly \twot\ solutions $u(x,t)$ over a 2-torus of
...
Smooth, spatially periodic velocity field $u$ %(x,t)=u(x+\speriod{},t)$
...
\bea
  \utensor_k(t) &=& \frac{1}{N} \sum^{N-1}_0 u_n(t) e^{-iq_kx_n}
  \,=\, \frac{1}{N} \sum^{N-1}_0 u_n(t) e^{-i 2 \pi k n /N}
  \,,\quad
...
\beq
%\dot{\utensor}_k(t)
\frac{d~}{dt}\utensor_k(t)
= ( q_k^2 - q_k^4 )\, \utensor_k(t)
- \frac{i q_k}{2} \!\sum_{k'=0}^{N-1} \!\!\utensor_{k'}(t) \utensor_{k-k'}(t)
\,.
\label{e-Fks}
\eeq
...
To calculate the temporal stability of a spatial \eqv\ $\utensor_\stagn$
...
%  \Mvar_{ij}(a_q)  = \left.\frac{\partial v_i}{\partial a_j}\right|_{a=a_q}
  ...
  \left.\frac{\partial \dot{\utensor}_i}{\partial \utensor_j}\right|_{\utensor={\utensor_\stagn}}
\,.
\label{eq:StabMat}
\eeq
For \KS\ we can
...
The \KS\ flat flame front $u(x,t)=0$ is always a temporal {\eqv} of \refeq{e-ks},
