...
tw10 = [4.35443487e-01,   2.55718139e-16,   3.55754017e-01, 3.69921902e-02, ...
...
[tw1, fval, info] = fsolve(ftw, tw10);%Get it precise
...
AAanalytic = gradV(tw1);
Aanalytic = gradVred(tw1);
%A = Aanalytic;
A = Aanalytic;
...
for phi = 0:2*(mu/nu)*pi/N:2*(mu/nu)*pi-2*(mu/nu)*pi/N
    ...
    [xsol, uu, tt] = ksETDRK4(x0, tf);
    
    fname = ['data/xsolfull', num2str(i), '.dat'];
    save(fname,'xsol');
    ...
    fname = ['data/xsolsspfull', num2str(i), '.dat'];
    save(fname,'xsolssp');
    delcomm = ["sed -i.bak -e '1,5d' ", fname]; %Delete first 5 lines of the text file
...
%fphi = @(phi) SliceCond(LieEl(phi)*tw2);
%[phi,fval,info] = fsolve(fphi, 0); %Get it back onto slice if it moved out
%tw2 = LieEl(phi)*tw2;
...
[xsol, uu, tt] = ksETDRK4(rpo0, 3*Trpo);

xsolrel = xsol - tw1';
xsolssp = ee'*xsolrel';
xsolssp = xsolssp';

fname = 'data/rposolsspfull.dat';
save(fname,'xsolssp');
delcomm = ["sed -i.bak -e '1,5d' ", fname]; %Delete first 5 lines of the text file
system(delcomm); %So that we can easily read it from python

%Compute group orbits:
gtw1=[]; gtw2=[];
for theta = 0:0.01:2*pi
	
	gtw1 = [gtw1; (LieEl(theta)*tw1)'];
	gtw2 = [gtw2; (LieEl(theta)*tw2)'];
	
end
%Project and save group orbits:
gtw1rel = gtw1 - tw1';
gtw1ssp = ee'*gtw1rel';
gtw1ssp = gtw1ssp';

fname = 'data/gtw1sspfull.dat';
save(fname,'gtw1ssp');
delcomm = ["sed -i.bak -e '1,5d' ", fname]; %Delete first 5 lines of the text file
system(delcomm); %So that we can easily read it from python
